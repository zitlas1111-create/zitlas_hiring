/* ═══════════════════════════════════════════════════
   ZITLAS — Nutritionist Apply (Scrollable Profile Form)
   nutritionist-apply.js
═══════════════════════════════════════════════════ */
...
(function () {
  ...
  /* ── Application-type guard ──────────────────────
     ...
     "Nutritionist". Anyone who lands here with a
     different (or missing) type is redirected back.   */
  (function () {
    var chosen = localStorage.getItem('zitlas_application_type');
    if (chosen && chosen !== 'nutritionist') {
  ...
  if (progHeader) {
    ...
    progHeader.innerHTML =
      '<div class="prog-inner">' +
        '<div class="prog-info">' +
          '<span class="prog-form-title">Nutritionist Application</span>' +
  ...
  var REQUIRED = [
    { type: 'input',  id: 'nutName'           },
    { type: 'input',  id: 'mobileNum'         },
    { type: 'input',  id: 'nutEmail'          },
    { type: 'chips',  group: 'gender'         },
    { type: 'input',  id: 'dob'              },
    { type: 'input',  id: 'nutCity'           },
    { type: 'chips',  group: 'qualification'  },
    { type: 'chips',  group: 'expYears'       },
    { type: 'chips',  group: 'isRD'           },
    { type: 'chips',  group: 'wellness'        },
    { type: 'chips',  group: 'clients'        },
    { type: 'chips',  group: 'expertise'      },
    { type: 'chips',  group: 'services'       },
    { type: 'chips',  group: 'mode'           },
    ...
    { type: 'textarea', id: 'nutBio'          },
    { type: 'chips',  group: 'clientCount'    },
    { type: 'chips',  group: 'hasTestimonials'},
  ...
  /* ── Input Listeners ────────────────────────────── */
  ['nutName','mobileNum','nutEmail','nutCity','nutBio','feeDayReview','feeWeekReview'].forEach(function (id) {
  ...
  bindFileLabel('nutCertFile',   'nutCertLabel');
  bindFileLabel('videoFile',     'videoLabel');
  bindFileLabel('testimonialFile','testimonialLabel');
  bindFileLabel('successFile',   'successLabel');
  bindFileLabel('degreeFile',    'degreeLabel');
  bindFileLabel('nutCertFile','nutCertLabel');
  bindFileLabel('achieveFile',   'achieveLabel');

  /* ── Bio Word Count ─────────────────────────────── */
  var bioEl    = document.getElementById('nutBio');
  ...
  /* ── Draft Save / Restore ───────────────────────── */
  var STORAGE_KEY = 'zitlas_nutritionist_apply';
  var TEXT_FIELDS = ['nutName','mobileNum','nutEmail','dob','nutCity','nutBio','feeDayReview','feeWeekReview','feeMonthly','achievements'];
  ...
  function submitToAffiliate(formData, onDone) {
    ...
    var payload = {
      name:          formData.nutName  || '',
      email:         formData.nutEmail || '',
  ...
  function saveDraft(silent) {
    ...
    if (silent) {
      ...
      submitToAffiliate(collectData(), function (appId) {
        ...
        window.location.href = '../../application-success.html?role=nutritionist';
      });
    }
  }
  ...
  function prefillFromUser() {
    ...
    fill('nutName',   cu.name  || '');
    fill('mobileNum', cu.phone || '');
    fill('nutEmail',  cu.email || '');

    if (cu.authMethod === 'google') {
      var emailEl = document.getElementById('nutEmail');
      if (emailEl) {
